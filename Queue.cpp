#include "Queue.h"
#include "vibor.h"
void vibor5(std::queue<Auto>& AutoQueue) {
    ...
    while (true) {
        ...
        case 4: {
            ...
            while (true) {
                std::string nomerStr;
                pos = 0;
                std::cout << "Введите номер автомобиля (от 1 до " << AutoQueue.size() << ") для перезаписи\n";
                std::cin >> nomerStr;
                nomer = safeStoi(nomerStr);
                if (nomer == -1) {
                    continue;
                }
                else if (nomer < 1 || nomer > AutoQueue.size()) {
            ...
            if (nomer < 1 || nomer > AutoQueue.size()) {
                std::cout << "Неверный номер автомобиля\n";
                break;
            }
